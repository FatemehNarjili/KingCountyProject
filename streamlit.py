...
# Sample data
king_county_df = pd.read_csv("/home/fatemeh/Documents/Projects/KingCountyProject/data/king_county_cleaned_data.csv",
                             low_memory=False, dtype={"Violation Code": "object"})
king_county_df["Inspection Date"] = pd.to_datetime(
    king_county_df["Inspection Date"])
not_null = king_county_df[~(king_county_df['Longitude'].isnull()) & ~(
    king_county_df['Grade'].isnull())]
...
def map_date(start_date, end_date, df):
    ...
    date_mask = (df['Inspection Date'] >= str(start_date)) & (
        df['Inspection Date'] <= str(end_date))
    ...
    for index, row in filtered_df.iterrows():
        ...
        folium.Marker(location=location, icon=Icon(
            color=color)).add_to(king_map)
    ...
    st.write("count of inspections :", len(
        filtered_df.groupby(by='Inspection_Serial_Num')))
    st.write("count of closed businesses :", len(
        filtered_df[filtered_df['Inspection Closed Business'] == 1].groupby(by='Inspection_Serial_Num')))

    filtered_df_closed_business = filtered_df[filtered_df["Inspection Closed Business"] == 1]
    show_most_closed_viol = pd.DataFrame(filtered_df_closed_business[[
                                         "Violation Code", "Violation Description", "Violation Points", "Violation Type"]].value_counts())
    st.dataframe(show_most_closed_viol)

    show_most_viol_record = pd.DataFrame(filtered_df[[
                                         "Violation Code", "Violation Description", "Violation Points", "Violation Type"]].value_counts())
    st.dataframe(show_most_viol_record)


not_null['grading_by_color'] = not_null.apply(map_score_to_color, axis=1)
gp = not_null.groupby('Inspection Date')[['Inspection Date', 'City', 'Latitude', 'Longitude', 'grading_by_color', 'Inspection Closed Business',
                                          'Inspection_Serial_Num', "Violation Code", "Violation Description", "Violation Points", "Violation Type"]].head()
...
s = pd.DataFrame(closed_business[["Violation Code", "Violation Description",
                 "Violation Points", "Violation Type"]].value_counts())
st.write(s)


plt.figure(figsize=(20, 5))
sns.countplot(data=closed_business, x="Violation Code", hue="Violation Type", palette=[
              "red", "blue"], dodge=False, order=closed_business["Violation Code"].value_counts().index)
...
def plot_top_10_violations(df, title, color):
    ...
    sns.barplot(data=top_10_counts, x="Violation Code",
                y="Count", palette=[color], ax=ax)
...
resampled_dates = newking.loc[:, [
    'Inspection Date', 'Inspection Closed Business']].copy()
st.write(resampled_dates)


resample_6M = resampled_dates.set_index('Inspection Date').resample(
    '6M')['Inspection Closed Business'].agg(['count', 'sum'])
plt.figure()
...
resample_Y = resampled_dates.set_index('Inspection Date').resample(
    'Y')['Inspection Closed Business'].agg(['count', 'sum'])
plt.figure()
...
table = king_county_df.groupby("Business_ID").agg({"City": "first", "Grade": "first", "Seating": "first",
                                                   "Risk": "first", "Inspection Date": "nunique"}).sort_values("Inspection Date", ascending=False)
...
while end_date <= king_county_df['Inspection Date'].max():
    subset = king_county_df[(king_county_df['Inspection Date'] >= start_date) & (
        king_county_df['Inspection Date'] < end_date)]
...
# pop_df = pd.read_csv("/home/fatemeh/Documents/Projects/KingCountyProject/data/us-cities-table.csv")
